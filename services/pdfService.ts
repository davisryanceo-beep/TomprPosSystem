...
export const printShiftReceipt = (log: CashDrawerLog) => {
  const doc = new jsPDF({
    orientation: 'portrait',
    unit: 'mm',
    format: [80, 150] // Receipt size format
  });

  doc.setFontSize(14);
  // doc.setFont("helvetica", "bold");
  doc.text(SHOP_NAME, 40, 10, { align: 'center' });
  
  doc.setFontSize(10);
  // doc.setFont("helvetica", "normal");
  doc.text("Shift Summary Receipt", 40, 16, { align: 'center' });
  ...
  doc.text(`Cashier: ${log.cashierName}`, 5, y); y += lineSpacing;
  doc.text(`Type: ${log.type}`, 5, y); y += lineSpacing;
  doc.text(`Shift Date: ${log.shiftDate}`, 5, y); y += lineSpacing;
  
  doc.line(5, y, 75, y); y += lineSpacing;
  
  doc.text(`Expected Total:`, 5, y);
  doc.text(formatCurrency(log.expectedAmount), 75, y, { align: 'right' }); y += lineSpacing;
  
  doc.text(`Declared Total:`, 5, y);
  doc.text(formatCurrency(log.declaredAmount), 75, y, { align: 'right' }); y += lineSpacing;

  if (log.declaredAmountUSD) {
    doc.text(`(Includes USD):`, 5, y);
    doc.text(`$${log.declaredAmountUSD.toFixed(2)}`, 75, y, { align: 'right' }); y += lineSpacing;
  }
  
  doc.line(5, y, 75, y); y += lineSpacing;
  
  // doc.setFont("helvetica", "bold");
  doc.text(`Variance:`, 5, y);
  doc.text(`${log.discrepancy > 0 ? '+' : ''}${formatCurrency(log.discrepancy)}`, 75, y, { align: 'right' }); y += lineSpacing;
  
  if (log.cashierNotes || log.adminNotes) {
    // doc.setFont("helvetica", "normal");
    y += lineSpacing;
    doc.text(`Notes:`, 5, y); y += lineSpacing;
    if (log.cashierNotes) {
      const splitNote = doc.splitTextToSize(`Cashier: ${log.cashierNotes}`, 70);
      doc.text(splitNote, 5, y);
      y += (splitNote.length * 4) + 2;
    }
    if (log.adminNotes) {
      const splitNote = doc.splitTextToSize(`Admin: ${log.adminNotes}`, 70);
      doc.text(splitNote, 5, y);
      y += (splitNote.length * 4) + 2;
    }
  }
  ...
  doc.text("Generated by Tompr POS", 40, y + 10, { align: 'center' });

  doc.save(`shift_receipt_${log.shiftDate}_${log.type}.pdf`);
};